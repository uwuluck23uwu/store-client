...
const EditBannerScreen = ({ route, navigation }: any) => {
  ...
  useEffect(() => {
    if (banner) {
      ...
      setImage({ uri: convertImageUrl(banner.imageUrl) });
    }
  }, [banner]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.isSuccess) {
        Alert.alert("สำเร็จ", "แก้ไขแบนเนอร์เรียบร้อยแล้ว", [
          {
            text: "ตกลง",
      ...
      Alert.alert(
        "เกิดข้อผิดพลาด",
        error?.data?.message || "ไม่สามารถแก้ไขแบนเนอร์ได้"
      );
    }
  };

  if (isFetching) {
    return <LoadingState message="กำลังโหลดข้อมูลแบนเนอร์..." />;
  }
